...
function renderComponent(node: ComponentNode, primaryColor: string): string {
...
export function renderSchemaToHtml(
  ...
  const body = componentTree.map((node) => renderComponent(node, primaryColor)).join("\n");
...
body{font-family:system-ui,-apple-system,sans-serif;color:#1e293b;background:#f1f5f9;padding:16px}
.doc{max-width:780px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,.08)}
...
.text-block,.table-block,.stats-block,.ai-summary,.profile-card,.signature,.chart-block,.timeline{padding:16px 28px;border-bottom:1px solid #f1f5f9}
...
td{padding:7px 10px;color:#334155;border-bottom:1px solid #f1f5f9}
...
.chart-block svg{overflow:visible;display:block;margin-top:8px}
