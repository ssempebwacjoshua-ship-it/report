  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <form onSubmit={(e) => void submit(e)} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">Enter your school code and account email. If the account exists, we will send a reset code.</p>
        {sent ? <p className="mt-4 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-700">If an account exists, we sent a reset code.</p> : null}
        {error ? <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}
        <label className="mt-5 block text-sm font-semibold text-slate-700" htmlFor="schoolCode">School code</label>
        <input id="schoolCode" required className="input mt-2" value={schoolCode} onChange={(e) => setSchoolCode(e.target.value)} disabled={loading || sent} />
        ...
        <button type="submit" className="btn btn-primary mt-5 w-full py-3" disabled={loading || sent}>{loading ? "Sending..." : "Send reset code"}</button>
      </form>
    </main>
  );
}
