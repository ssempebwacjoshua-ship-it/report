...
export function SmartPagesBillingPage() {
  ...
  // Resolve merchant code from config or defaults
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div>
        ...
        <p className="mt-0.5 text-sm text-slate-500">
          Manage your Smart Pages credits and payment history.
        </p>
      </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-3xl font-black text-slate-950">
                {summary.remainingCredits === Infinity
                  ? "Unlimited"
                  : summary.remainingCredits.toLocaleString()}
              </p>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 border-t border-slate-100 pt-4">
            <div>
              ...
              <p className="mt-0.5 font-bold text-slate-800">
                {summary.includedCredits === Infinity ? "∞" : summary.includedCredits.toLocaleString()}
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-500">Top-up</p>
              <p className="mt-0.5 font-bold text-slate-800">
                {summary.topUpCredits.toLocaleString()}
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-500">Used</p>
              <p className="mt-0.5 font-bold text-slate-800">{summary.usedCredits.toLocaleString()}</p>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="px-4 py-2.5 text-right text-xs font-bold text-slate-500">Credits</th>
      ...
      {/* Recent usage */}
      <div>
        <h2 className="mb-3 text-sm font-black uppercase tracking-wide text-slate-700">Recent Credit Usage</h2>
        {ledger.length === 0 ? (
          <div className="rounded-xl border border-slate-100 bg-white py-8 text-center">
            <p className="text-sm text-slate-400">No credit usage recorded yet.</p>
          </div>
        ) : (
          <div className="overflow-hidden rounded-xl border border-slate-100 bg-white">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="px-4 py-2.5 text-right text-xs font-bold text-slate-500">Credits used</th>
      ...
      {buyStep ? (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-4 sm:items-center">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
            ...
            {buyStep === "package" && (
              <div className="p-5">
                <h2 className="text-base font-black text-slate-900">Buy Credits</h2>
                <p className="mt-1 text-sm text-slate-500">Choose a credit package.</p>
            ...
            {buyStep === "receipt" && activePayment && selectedPackage && (
              <div className="p-5">
                ...
                <h2 className="text-base font-black text-slate-900">Enter Payment Details</h2>
                <p className="mt-1 text-sm text-slate-500">
            ...
            {buyStep === "done" && (
              <div className="p-6 text-center">
                <div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-full bg-emerald-100">
                  <svg viewBox="0 0 24 24" className="h-6 w-6 text-emerald-600" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                    <path d="M20 6 9 17l-5-5" />
                  </svg>
                </div>
                <h2 className="text-base font-black text-slate-900">Payment Submitted</h2>
                <p className="mt-2 text-sm text-slate-500">
                  Credits will be added to your account after confirmation. This usually takes 1–2 business hours.
                </p>
                <button
                  type="button"
