  ...
  return (
    <section className="report-print-area min-w-0">
      ...
      <div className="report-card-sheet mx-auto max-w-4xl overflow-hidden rounded-2xl bg-white shadow-[0_4px_24px_rgba(0,0,0,0.12)] ring-1 ring-slate-200">
        ...
        {/* 1. Official header */}
        <div className="report-header-bg bg-[#0f2a5e] px-8 py-6 text-white">
          <div className="flex items-center gap-5">
            <ShieldIcon />
            <div className="flex-1 text-center">
              <h1 className="text-2xl font-bold tracking-wide">Uganda High School</h1>
              <p className="mt-0.5 text-sm font-medium uppercase tracking-widest text-blue-200">
                Student Academic Report
              </p>
            </div>
            <div className="w-24 flex-shrink-0 text-right text-xs leading-relaxed text-blue-200">
              <div className="font-semibold text-white">{card.academicYear}</div>
              <div>{card.term}</div>
            </div>
          </div>
          <div className="mt-5 grid grid-cols-3 gap-2 border-t border-blue-800 pt-4 text-center text-xs text-blue-100">
            <div>
        ...
        <div className="px-8 py-6">
          ...
          {/* 3. Academic summary */}
          <div className="report-summary-cards mb-6 grid grid-cols-2 gap-3 text-center lg:grid-cols-4">
            <div className="report-summary-card rounded-xl bg-blue-600 p-4 text-white">
              <b className="block text-3xl font-bold tabular-nums">{card.average ?? "—"}</b>
              <span className="mt-1 block text-xs font-semibold uppercase tracking-wider text-blue-100">
                Average
              </span>
            </div>
            <div className="report-summary-card rounded-xl bg-[#4c1d95] p-4 text-white">
              <b className="block text-3xl font-bold">{card.grade ?? "—"}</b>
              <span className="mt-1 block text-xs font-semibold uppercase tracking-wider text-purple-200">
                Grade
              </span>
            </div>
            <div className="report-summary-card rounded-xl bg-emerald-600 p-4 text-white">
              <b className="block text-3xl font-bold tabular-nums">
                {card.overallPosition != null ? `#${card.overallPosition}` : "—"}
              </b>
              <span className="mt-1 block text-xs font-semibold uppercase tracking-wider text-emerald-100">
                Position
              </span>
            </div>
            <div className="report-summary-card rounded-xl bg-amber-500 p-4 text-white">
              <b className="block text-3xl font-bold tabular-nums">{card.missingMarks.length}</b>
              <span className="mt-1 block text-xs font-semibold uppercase tracking-wider text-amber-100">
                Missing
              </span>
            </div>
          </div>
          ...
          <div className="report-comments mb-6">
            <h3 className="mb-3 text-xs font-bold uppercase tracking-widest text-slate-400">
              Comments &amp; Signatures
            </h3>
            <div className="grid gap-4 sm:grid-cols-3">
              {(
                ...
                <div key={title} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <div className="mb-4 min-h-[56px] rounded-lg border border-dashed border-slate-200 bg-slate-50 p-2 text-xs italic text-slate-400">
                    {title === "Class Teacher's Comment" && card.comments ? card.comments : "—"}
                  </div>
                  <div className="space-y-3 text-xs text-slate-500">
                    <div className="flex items-end gap-2">
                      <span className="w-10 flex-shrink-0">Name:</span>
                      <div className="flex-1 border-b border-slate-300" />
                    </div>
                    <div className="flex items-end gap-2">
                      <span className="w-10 flex-shrink-0">Sign:</span>
                      <div className="flex-1 border-b border-slate-300" />
                    </div>
                    <div className="flex items-end gap-2">
                      <span className="w-10 flex-shrink-0">Date:</span>
                      <div className="flex-1 border-b border-slate-300" />
                    </div>
                  </div>
                </div>
          ...
          {/* 7. Footer */}
          <div className="report-footer border-t border-slate-200 pt-4 text-center text-xs text-slate-400">
